...
class CalendarPage extends StatefulWidget {
  ...
  const CalendarPage({super.key, this.initialRole});
...
class _CalendarPageState extends State<CalendarPage> {
  ...
  Future<void> _initUserAndLoadTasks() async {
    // 1. Use explicitly provided role first, then infer from route
    UserRole? routeRole = widget.initialRole;
    debugPrint('[CalendarPage] widget.initialRole: ${widget.initialRole}');
    if (routeRole == null) {
      try {
        final routeName = ModalRoute.of(context)?.settings.name;
        debugPrint('[CalendarPage] Route name: $routeName');
        if (routeName != null) {
          if (routeName.startsWith('/production'))
            routeRole = UserRole.production;
          else if (routeName.startsWith('/admin'))
            routeRole = UserRole.admin;
          else if (routeName.startsWith('/accounts'))
            routeRole = UserRole.accounts;
          else if (routeName.startsWith('/lab_testing'))
            routeRole = UserRole.lab_testing;
        }
      } catch (_) {
        // ignore context errors
      }
    }
    debugPrint('[CalendarPage] routeRole after processing: $routeRole');
    ...
    try {
      ...
      if (user != null) {
        ...
        if (res != null) {
          ...
          // Use route role if available (determines which section the user is in),
          // otherwise fall back to database role
          if (routeRole != null) {
            _role = routeRole;
          } else {
    ...
    debugPrint('[CalendarPage] Final _role set to: $_role');
    await _loadTasks();
  }
    ...
    try {
      ...
      for (final s in shipped) {
        ...
        extra.add(
          CalendarTask(
            title: 'Shipped: #${s.orderNumber ?? ''}',
            description:
                'shipped|order:${s.id}|client:${s.clientName ?? ''}|products:${s.productsList ?? ''}',
            date: dt,
            category: TaskCategory.production,
          ),
        );
      ...
      for (final p in purchases) {
        ...
        extra.add(
          CalendarTask(
            ...
            description:
                'purchase|id:${p.id}|material:${p.material}|amount:${p.totalAmount?.toStringAsFixed(0) ?? '0'}',
            date: p.purchaseDate!,
            category: TaskCategory.accounts,
          ),
        );
      ...
      for (final task in tasks) {
        ...
        if (task.isRecurring && task.parentTaskId == null) {
          final instances = task.generateRecurringInstances(
            rangeStart,
            rangeEnd,
          );
  ...
  bool _matchesFilters(CalendarTask t) {
    // 0. Role-Based Visibility (Security)
    // Admin sees everything. Others see only tasks relevant to them.
    if (_role != UserRole.admin) {
      final roleName = _role!.name.toLowerCase();
      final assignedToCategory = t.category.name.toLowerCase();
      final assignedBy = t.assignedBy?.toLowerCase();

      bool canSee = false;

      // 1. Task is assigned TO my department
      if (assignedToCategory == roleName) canSee = true;

      // 2. Task was assigned BY my department (or me)
      if (assignedBy == roleName) canSee = true;

      // 3. Fallback: If assignedBy is a UUID, check against current user ID
      if (assignedBy != null && assignedBy == _currentUserAuthId) canSee = true;
    ...
    // Updated logic: Filter matches if task is assigned TO that category OR assigned BY that category
    if (_selectedFilters.contains('Admin') && !isExtra) {
      if (t.category == TaskCategory.admin) return true;
      if (t.assignedBy?.toLowerCase() == 'admin') return true;
    }
    
    if (_selectedFilters.contains('Production') && !isExtra) {
      if (t.category == TaskCategory.production) return true;
      if (t.assignedBy?.toLowerCase() == 'production') return true;
    }
    
    if (_selectedFilters.contains('Accounts') && !isExtra) {
      if (t.category == TaskCategory.accounts) return true;
      if (t.assignedBy?.toLowerCase() == 'accounts') return true;
    }
  ...
  Future<void> _toggleComplete(int idx) async {
    ...
    if (task.id == null) {
      final newTask = CalendarTask(
        ...
        isRecurring:
            false, // Individual instance is not itself recurring usually
      );
  ...
  List<CalendarTask> _getTasksForDay(DateTime day) {
    ...
    return combined.where((t) {
      // 1. Role-Based Visibility (Security)
      if (_role != UserRole.admin) {
        final roleName = _role!.name.toLowerCase();
        final assignedToCategory = t.category.name.toLowerCase();
        final assignedBy = t.assignedBy?.toLowerCase();
        ...
        // Task is assigned TO my department
        if (assignedToCategory == roleName) canSee = true;

        // Task was assigned BY my department (or me)
        if (assignedBy == roleName) canSee = true;

        // Fallback: If assignedBy is a UUID, check against current user ID
        if (assignedBy != null && assignedBy == _currentUserAuthId) canSee = true;
      ...
      // Category-based filters (Admin/Production/Accounts) using updated logic
      if (_selectedFilters.contains('Admin') && !isExtra) {
        if (t.category == TaskCategory.admin) return true;
        if (t.assignedBy?.toLowerCase() == 'admin') return true;
      }
  ...
  void _showManageRecurringTasksDialog() {
    final recurringTasks =
        _tasks.where((t) => t.isRecurring && t.parentTaskId == null).toList();
                            ...
                            : ConstrainedBox(
                              ...
                              child: ListView.separated(
                                ...
                                itemBuilder: (_, idx) {
                                  ...
                                  return InkWell(
                                    onTap:
                                        () => _showRecurringTaskDetailsDialog(
                                          ctx,
                                          t,
                                        ),
                                      ...
                                      child: Row(
                                        children: [
                                          ...
                                          IconButton(
                                            ...
                                            onPressed:
                                                () =>
                                                    _showRecurringDeleteOptionsDialog(
                                                      ctx,
                                                      t,
                                                    ),
                                            tooltip: 'Delete',
            ...
            content: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 4),
                  Text(
                    '${_getRecurrenceLabel(t.recurrenceType)} every ${t.recurrenceInterval}',
                  ),
                  if (t.recurrenceEndDate != null) ...[
                    const SizedBox(height: 8),
                    Text(
  ...
  void _showRecurringDeleteOptionsDialog(
                  ...
                  onTap: () async {
                    ...
                    final tasksToSkip =
                        _tasks.where((task) {
                          ...
                          return !task.date.isAfter(weekEnd) &&
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                        content: Text(
                          allSuccess
                              ? 'Skipped $skippedCount tasks for the next 7 days'
                              : 'Some tasks could not be skipped',
                        ),
                      ),
                    );
                  },
                        ...
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              ...
                              Text(
                                'Hide all occurrences for the next 7 days',
                                style: TextStyle(
                                  fontSize: 12,
                  ...
                  onTap: () async {
                    ...
                    if (parentId != null) {
                      ...
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                          content: Text(
                            'Use "Skip for this week" to hide this occurrence',
                          ),
                        ),
                      );
                    }
              ...
              if (confirm == true) {
                ...
                if (success) {
                  ...
                  if (mounted) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                        content: Text('Failed: ${_tasksService.lastError}'),
                      ),
                    );
                  }
                }
  ...
  Widget _taskTile(CalendarTask t, int originalIndex) {
    ...
    } else if (t.description.startsWith('installment-paid') ||
        t.description.startsWith('advance-paid')) {
                          ...
                          if (!isExtra)
                            SizedBox(
                              ...
                              child: Checkbox(
                                ...
                                onChanged:
                                    canEdit
                                        ? (_) => _toggleComplete(originalIndex)
                                        : null,
  ...
  void _showTaskDetailsDialog(CalendarTask t) {
    ...
    if (t.description.startsWith('installment-paid|') ||
        ...
        t.description.startsWith('shipped|')) {
      final parts = t.description.split('|');
      final orderPart = parts.firstWhere(
        (p) => p.startsWith('order:'),
        orElse: () => '',
      );
                          ...
                          if (!isExtra) ...[
                            ...
                            Row(
                              children: [
                                ...
                                const SizedBox(width: 8),
                                Text(
                                  'Assigned by: ${_sentenceCase(t.assignedBy ?? 'Admin')}',
                                  style: TextStyle(
                                    fontSize: 13,
                                    color: Colors.grey.shade700,
                                  ),
                    ...
                    child: SingleChildScrollView(
                      child: Column(
                        children: [
                          ...
                          _detailField(
                            'Quantity',
                            purchase.quantity?.toStringAsFixed(2) ?? '0.00',
                          ),
                          _detailField(
                            'Cost per Unit',
                            '₹${purchase.cost?.toStringAsFixed(0) ?? '0'}',
                          ),
                          _detailField(
                            'Total Amount',
                            '₹${purchase.totalAmount?.toStringAsFixed(0) ?? '0'}',
                            isAmount: true,
                          ),
                          _detailField(
                            'Payment Status',
                            (purchase.paymentStatus ?? 'unpaid').toUpperCase(),
                            isStatus: true,
                          ),
                          _detailField(
                            'Payment Due Date',
                            purchase.paymentDueDate != null
                                ? DateFormat(
                                  'yyyy-MM-dd',
                                ).format(purchase.paymentDueDate!)
                                : 'N/A',
                          ),
                        ],
                      ),
  ...
  void _showOrderTaskDialog(int orderId, String type) async {
    ...
    final advances = await AdvancePaymentsService.instance.getPaymentsForOrder(
      orderId,
    );

    final totalAdvances = advances.fold(0.0, (sum, a) => sum + a.amount);
    final totalPaidInstallments = installments
        .where((i) => i.isPaid)
                    ...
                    child: ElevatedButton.icon(
                      onPressed: () {
                        ...
                        Navigator.push(
                          ...
                          MaterialPageRoute(
                            builder:
                                (c) => AccountsOrdersPage(filterId: order.id),
                          ),
                        );
                      },
                      icon: const Icon(Icons.open_in_new),
        ...
        children: [
          ...
          Container(
            ...
            decoration: BoxDecoration(
              color:
                  Colors
                      .white, // simplified for cleaner look according to image
              borderRadius: BorderRadius.circular(10),
              border: Border.all(color: Colors.grey.shade200),
            ),
            child: Text(
                ...
                Container(
                  ...
                  child: Row(
                    children: [
                      ...
                      Text(
                        _sentenceCase(
                          (widget.currentUserRole ?? UserRole.production).name,
                        ),
                        style: TextStyle(
                          fontSize: 13,
                          fontWeight: FontWeight.bold,
                          color: _getRoleColor(
                            (widget.currentUserRole ?? UserRole.production)
                                .name,
                          ),
                        ),
                      ),
                              ...
                              IconButton(
                                ...
                                onPressed: () async {
                                  final d = await showDatePicker(
                                    ...
                                    initialDate:
                                        _recurrenceEndDate ??
                                        _selDate!.add(const Duration(days: 30)),
                                    firstDate: _selDate!,
                                    lastDate: DateTime(2030),
                                  );
                                  if (d != null)
        ...
        ElevatedButton(
          ...
          onPressed: () {
            ...
            final assignedByRole =
                (widget.currentUserRole ?? UserRole.production).name;
            debugPrint(
              '[AddTaskDialog] Creating task with assignedBy: $assignedByRole, currentUserRole: ${widget.currentUserRole}',
            );
            ...
            widget.onTaskAdded(
              CalendarTask(
                ...
                date: _selDate!,
                category: _cat,
                assignedBy: assignedByRole, // From: logged-in user
